...
public class Call {
...

	{
		...
		String usuario = "wlsfernandes@gmail.com";
		String senha = "Kraf140298";

		// Chamadas.simple(usuario, senha);

		// Lista os MO nao lidos da plataforma, apos executar este metodo, o
		// mesmo vai marcar como lido os MOs, portanto para
		// continuar seus testes, va ate o painel de controle da Facilita,
		// Mensagens Recebidas, e Marque todos como NAO LIDOS.
		Call.listaMensagensRecebidas(usuario, senha);

		// Verifica a quantidade de creditos de sua conta
		Call.checkCredits(usuario, senha);
	}
	...
	public static void simple(String usuario, String senha) throws Exception {
		...
		sms.setDestinatario("21989392008");
		sms.setMessage("Nossa poc esta no ar mandando sms !!!!");
		// sms.setAno(2015);
	 ...
	 * @throws Exception
	 */
	public static void sms2infusion(String usuario, String senha, String phone, String contato) throws Exception {
		// Simple Send
		SmsSimples sms = new SmsSimples();
		sms.setUser(usuario);
		sms.setPassword(senha);
		sms.setDestinatario(phone);
		sms.setMessage("Ola "+ contato + " Teste de Integracao Infusion2SMS !!!!");
		// sms.setAno(2015);
		Retorno retorno = SendMessage.simpleSend(sms);
		System.out.println("Codigo:" + retorno.getCodigo());
		System.out.println("Descricao:" + retorno.getMensagem());
	...
	/**
	 * Lista os MO nao lidos da plataforma, apos executar este metodo, o mesmo
	 * vai marcar como lido os MOs, portanto para continuar seus testes, va ate
	 * o painel de controle da Facilita, Mensagens Recebidas, e Marque todos
	 * como NAO LIDOS.
		...
		try {
			...
			if (caixaEntrada != null && caixaEntrada.size() > 0) {
				for (MO mo : caixaEntrada) {
					...
					System.out
							.println("Data/Hora:" + new SimpleDateFormat("dd/MM/yyyy kk:mm").format(mo.getDataHora()));
	...
	/**
	 * Verifica exatamente quantos creditos possui em sua conta no momento que
	 * for feita a requisicao
	 * https://www.facilitamovel.com.br/manuais/IntegracaoHTTPS.pdf
	 */
